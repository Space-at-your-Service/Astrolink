...
<script>
  ...
  export default {
    ...
    props: {
      ...
      socketURL: {
        type: String,
        default: 'https://signaling.simplewebrtc.com:443/'
      },
      cameraHeight: {
        type: [Number, String],
        default: 160
    ...
    mounted() {
      ...
      this.rtcmConnection.session.audio=true;
    ...
    methods: {
      ...
      leave() {
          console.log(this.rtcmConnection.attachStreams)
    ...
    muteMyself(muted){
       // var firstRemoteStream = this.rtcmConnection.streamEvents.selectFirst({ remote: true }).stream;
            //    console.log(firstRemoteStream)

        if(muted){
        this.rtcmConnection.attachStreams[0].unmute('audio');
        muted=false
        }
        else {
        this.rtcmConnection.attachStreams[0].mute('audio');
        muted = true
        }
      ...
      shareScreen() {
        ...
        if (navigator.getDisplayMedia || navigator.mediaDevices.getDisplayMedia) {
          ...
          if (navigator.mediaDevices.getDisplayMedia) {
            this.getDisplayMedia({video: true, audio: false}).then(stream => {
              this.onGettingSteam(stream, that);
